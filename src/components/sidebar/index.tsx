...
export const Nav: React.FC<SideBarProps> = function Nav(props) {
  ...
  const handleInputKey = useCallback(() => {
    ...
    apiKeyRef.current = Modal.info({
      header: <div className="py-6 font-semibold">TIMUR AI KLJUC</div>,
  ...
  return (
    <div className={`flex h-full w-full flex-1 items-start border-white/20 ${isMobile ? 'scrollbar-trigger' : ''}`}>
      <nav className="flex h-full flex-1 flex-col space-y-1 p-2">
        ...
        {chatList.length > 0 && (
          <Popconfirm
            title="Da li ste sigurni da želite obrisati sve konverzacije?"
            content="Nakon brisanja sve konverzacije se zauvijek brisu"
            onConfirm={() => {
              handleDeleteAll();
              Toast.success('Uspješno');
            }}
